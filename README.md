...
| `--backend <name>`  | 백엔드 통합 선택 프롬프트를 건너뛰고 사용할 통합을 지정합니다. 선택지는 `none`, `supabase`입니다. 기본값은 `none`입니다. |
...
Supabase 통합을 선택하면 다음 파일과 설정이 추가됩니다.

- `@supabase/supabase-js`
- `app/lib/supabase.ts`
- `.env.example` (`VITE_SUPABASE_URL`, `VITE_SUPABASE_PUBLISHABLE_KEY`)
- `app/env.d.ts`의 Supabase 환경 변수 타입
...
Supabase 통합 생성 경로를 확인하려면:

```bash
node src/index.js /tmp/create-frontend-supabase-smoke --template react-router-ts --backend supabase --no-install --no-git --overwrite
node src/index.js /tmp/create-frontend-next-supabase-smoke --template next-oxc --backend supabase --no-install --no-git --overwrite
```
...
```bash
pnpm dlx /tmp/mobius29-create-frontend-0.1.0.tgz my-app --template react-router-ts --backend supabase --no-install --no-git
```
